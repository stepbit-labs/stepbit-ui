...
pub fn build_shell_bootstrap_script(session_id: &str, token: &str, api_base_url: &str) -> String {
    format!(
        concat!(
            ...
            "__stepbit_workspace_request() {{ local target=\"${{1:-.}}\"; local index_flag=\"${{2:-false}}\"; local resolved; resolved=$(__stepbit_resolve_path \"$target\") || {{ print -u2 -- \"stepbit: path not found: $target\"; return 1; }}; local response; response=$(curl -fsS -X POST \"$STEPBIT_TERMINAL_API_BASE/api/terminal/sessions/$STEPBIT_TERMINAL_SESSION_ID/workspace-load\" -H \"x-stepbit-terminal-token: $STEPBIT_TERMINAL_TOKEN\" -H 'content-type: application/json' -d \"{{\\\"path\\\":\\\"$resolved\\\",\\\"index\\\":$index_flag}}\") || {{ print -u2 -- \"stepbit: failed to load workspace\"; return 1; }}; local workspace_id=\"\" workspace_name=\"\" workspace_root=\"\" indexing_started=\"0\" already_registered=\"0\"; while IFS='=' read -r key value; do case \"$key\" in WORKSPACE_ID) workspace_id=\"$value\" ;; WORKSPACE_NAME) workspace_name=\"$value\" ;; WORKSPACE_ROOT) workspace_root=\"$value\" ;; INDEXING_STARTED) indexing_started=\"$value\" ;; ALREADY_REGISTERED) already_registered=\"$value\" ;; esac; done <<< \"$response\"; if [[ -z \"$workspace_name\" ]]; then workspace_name=\"workspace\"; fi; if [[ \"$already_registered\" == \"1\" ]]; then print -- \"[stepbit] switched to $workspace_name ($workspace_root)\"; else print -- \"[stepbit] loaded $workspace_name ($workspace_root)\"; fi; if [[ \"$indexing_started\" == \"1\" ]]; then print -- \"[stepbit] indexing started in background\"; else print -- \"[stepbit] indexing skipped; run 'stepbit-index [path]' when needed\"; fi; }}\r",
            "stepbit() {{ __stepbit_workspace_request \"${{1:-.}}\" false; }}\r",
            "stepbit-index() {{ __stepbit_workspace_request \"${{1:-.}}\" true; }}\r",
            "workspace() {{ case \"$1\" in load|'') shift 2>/dev/null || true; stepbit \"${{1:-.}}\" ;; index) shift 2>/dev/null || true; stepbit-index \"${{1:-.}}\" ;; *) print -u2 -- 'usage: workspace load [path] | workspace index [path]'; return 1 ;; esac; }}\r",
